...
if ! [[ -x "$(command -v gradle)" ]]; then
    echo "Installing gradle 8.2.1"
    curl -o gradle.zip -L https://downloads.gradle-dn.com/distributions/gradle-8.2.1-bin.zip
    mkdir /opt/gradle
    unzip -d /opt/gradle gradle.zip
    echo 'export PATH=/opt/gradle/gradle-8.2.1/bin:$PATH' >> ~/.bash_profile
    rm -rf gradle.zip
fi
