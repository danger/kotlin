...
if ! [ -x "$(command -v kotlinc)" ]; then
    ...
    curl https://github.com/JetBrains/kotlin/releases/download/v1.3.50/kotlin-compiler-1.3.50.zip
    mkdir /opt/kotlin
...
if ! [ -x "$(command -v gradle)" ]; then
    ...
    curl -q https://downloads.gradle-dn.com/distributions/gradle-5.6.2-bin.zip
    mkdir /opt/gradle
