...
if ! [[ -x "$(command -v kotlinc)" ]]; then
    echo "Installing kotlin compiler 1.6.0"
    curl -o kotlin-compiler.zip -L https://github.com/JetBrains/kotlin/releases/download/v1.6.0/kotlin-compiler-1.6.0.zip
...
if ! [[ -x "$(command -v gradle)" ]]; then
    echo "Installing gradle 7.3"
    curl -o gradle.zip -L https://downloads.gradle-dn.com/distributions/gradle-7.3-bin.zip
    mkdir /opt/gradle
    unzip -d /opt/gradle gradle.zip
    echo 'export PATH=/opt/gradle/gradle-7.3/bin:$PATH' >> ~/.bash_profile
    rm -rf gradle.zip
fi
