...
if ! [[ -x "$(command -v kotlinc)" ]]; then
    ...
    echo 'export PATH=/usr/local/kotlinc/bin:$PATH' >> ~/.bashrc
...
if ! [[ -x "$(command -v gradle)" ]]; then
    ...
    echo 'export PATH=/opt/gradle/gradle-5.6.2/bin:$PATH' >> ~/.bashrc
    rm -rf gradle.zip
fi

git clone https://github.com/danger/kotlin.git --single-branch --branch intellij-compat --depth 1 _danger-kotlin
